...
export function PublicHeader() {
  ...
  const desktopLink = ({ isActive }: { isActive: boolean }) => cn(
    'inline-flex min-h-11 items-center justify-center rounded-full border border-transparent px-5 text-sm font-semibold text-[color:var(--txt1)] transition hover:bg-white/80 hover:text-on-surface focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary',
    isActive && 'border-charcoal/65 bg-white/90 text-on-surface shadow-tactile-sm',
  );
  const mobileLink = ({ isActive }: { isActive: boolean }) => cn(
    'inline-flex min-h-10 flex-1 items-center justify-center rounded-2xl border border-outline-variant bg-white/80 px-4 text-sm font-semibold text-[color:var(--txt1)] transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary',
  ...
  return (
    <>
      <nav className="sticky top-0 z-40 hidden w-full border-b border-charcoal/10 bg-background/95 shadow-[0_8px_24px_rgba(28,28,24,0.05)] backdrop-blur-xl md:block">
        <div className={`mx-auto grid min-h-[72px] w-full grid-cols-[auto_1fr_auto] items-center gap-6 px-4 sm:px-6 lg:px-8 ${publicSurfaceWidth}`}>
          ...
          <div className="justify-self-end">
            <Button size="md" onClick={() => openAuth()}>Login / Signup</Button>
          </div>
        </div>
      </nav>
      ...
      <div className="sticky top-0 z-40 border-b border-charcoal/10 bg-background/95 backdrop-blur-xl md:hidden">
        <div className={`mx-auto flex min-h-[60px] w-full items-center gap-3 px-4 sm:px-6 ${publicSurfaceWidth}`}>
          ...
          <div className="ml-auto">
            <Button size="md" variant="link" onClick={() => openAuth()}>Login</Button>
          </div>
        </div>
      </div>
