...
const notFoundSurfaceWidth = 'w-full max-w-[1360px]';
const notFoundBodyClass = 'text-base leading-7 text-[color:var(--txt2)] lg:text-lg lg:leading-8';
const notFoundCardShell = 'rounded-[1.75rem] border-[1.5px] border-charcoal/75 shadow-tactile';

type NotFoundActionVariant = 'primary' | 'secondary' | 'link';
type NotFoundActionSize = 'md' | 'lg';
...
function NotFoundAction({
  ...
  className?: string;
  onClick?: () => void;
  size?: NotFoundActionSize;
  to?: string;
  variant?: NotFoundActionVariant;
}) {
  const sizeClass =
    variant === 'link'
      ? size === 'lg'
...
function NotFoundHeader({ onOpenAuth }: { onOpenAuth: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-charcoal/10 bg-background/92 backdrop-blur-xl">
      <div className={`mx-auto w-full ${notFoundSurfaceWidth} px-4 sm:px-6 lg:px-8`}>
        <div className="hidden min-h-[78px] items-center justify-between gap-6 md:flex">
          ...
          <div className="flex items-center gap-3 lg:gap-4">
            ...
            <NotFoundAction className="px-5 lg:px-6" onClick={onOpenAuth} size="md">
              Login / Signup
            </NotFoundAction>
          </div>
        </div>
        ...
        <div className="grid gap-3 py-3 md:hidden">
          <div className="flex min-h-[52px] items-center justify-between gap-3">
            ...
            <div className="ml-auto shrink-0">
              <NotFoundAction className="px-4" onClick={onOpenAuth} size="md" variant="link">
                Login
              </NotFoundAction>
            </div>
          </div>
...
function NotFoundFooter() {
  return (
    <footer className="mt-auto w-full border-t border-charcoal/10 bg-charcoal text-white/90">
      <div className={`mx-auto w-full ${notFoundSurfaceWidth} px-4 py-6 sm:px-6 lg:px-8 lg:py-7`}>
...
export function NotFoundPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <NotFoundHeader onOpenAuth={() => openAuth()} />

      <main className="flex w-full flex-1 justify-center overflow-x-clip px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-12">
        <section className={`mx-auto flex min-h-[calc(100vh-13rem)] w-full items-center justify-center ${notFoundSurfaceWidth}`}>
          <div className="relative w-full max-w-[36rem] py-12 text-center">
            ...
            <div className="relative z-10 grid justify-items-center gap-7 sm:gap-8">
              ...
              <p className={`max-w-[26rem] ${notFoundBodyClass}`}>
                The page slipped out of the map, but the rest of meowfolio is still right where you left it.
              </p>

              <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
                <NotFoundAction to={routes.home} size="lg">
                  Back Home
                </NotFoundAction>
                <NotFoundAction to={routes.about} variant="secondary" size="lg">
                  Read About
                </NotFoundAction>
              </div>
            </div>

            <div className={cn('pointer-events-none absolute inset-0 -z-10 rounded-[2rem] bg-white/60', notFoundCardShell)} />
          </div>
        </section>
      </main>
