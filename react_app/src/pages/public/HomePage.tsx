...
function RoundRailButton({
  ...
  return (
    <button
      type="button"
      className="grid size-10 shrink-0 place-items-center rounded-[9999px] border-[1.5px] border-charcoal/75 bg-white/90 text-lg text-on-surface shadow-tactile-sm transition hover:-translate-x-px hover:-translate-y-px hover:shadow-tactile sm:size-11"
      onClick={onClick}
      aria-label={label}
    >
...
export function HomePage() {
  ...
  return (
    <>
      <PublicSection className={`${publicPageSection} relative overflow-hidden`}>
        <div className="absolute left-12 top-12 h-40 w-40 rounded-full bg-primary-fixed blur-3xl" />
        <div className="absolute bottom-10 right-16 h-48 w-48 rounded-full bg-secondary-fixed blur-3xl" />
        ...
        <div className="relative z-10 grid items-center gap-10 lg:grid-cols-[minmax(0,0.88fr)_minmax(0,1.12fr)] lg:gap-14 xl:gap-20">
          <div className="grid content-start gap-6 lg:pr-6">
            ...
            <div className="grid gap-3 sm:gap-4">
              ...
              <p className={`max-w-2xl ${publicBodyClass}`}>
                Build the resume in one workspace, generate TeX-backed PDFs, and carry the same structured profile into ATS
                and JD analysis without jumping between disconnected tools.
              </p>
            </div>

            <div className="flex flex-wrap items-center gap-3 sm:gap-4">
              <Button className="px-8" onClick={openHomeAuth} size="lg">
                Get Started
              </Button>
              <Button className="px-8" size="lg" to={routes.about} variant="secondary">
                Read the story
              </Button>
            </div>
          </div>
          ...
          <div className="relative mx-auto min-w-0 w-full max-w-[52rem] lg:ml-auto">
            <div className="absolute -left-4 top-8 hidden h-[86%] w-[88%] rounded-[2rem] bg-primary-fixed lg:block" />
      ...
      <PublicSection className={publicPageSection}>
        ...
        <div className="hide-scrollbar flex max-w-full snap-x snap-mandatory gap-5 overflow-x-auto pb-3 lg:gap-6" ref={storyRailRef}>
          {storyCards.map((card, index) => (
            <article
              className={`grid w-[18rem] flex-none snap-start content-start gap-4 bg-white/90 p-5 sm:w-[19rem] sm:p-6 lg:w-[20rem] ${publicCardShell}`}
              key={card.title}
            >
              ...
              <p className="font-headline text-xl font-extrabold leading-tight text-on-surface sm:text-2xl">
                {card.title}
              </p>
            </article>
          ))}
          ...
          <article className="flex w-[18rem] flex-none snap-start flex-col items-center justify-center gap-6 rounded-[1.75rem] border-[1.5px] border-charcoal/75 bg-coral p-8 text-center shadow-tactile sm:w-[19rem] lg:w-[20rem]">
            ...
            <Button className="bg-white text-on-surface hover:text-primary" onClick={openHomeAuth} size="lg">
              Get Started
            </Button>
          </article>
        </div>
      ...
      <PublicSection className={publicPageSection}>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {features.map((feature) => (
            <article
              className={`grid h-full content-start gap-5 bg-white/90 p-6 transition hover:-translate-x-px hover:-translate-y-px sm:p-7 md:min-h-[17rem] ${publicCardShell}`}
              key={feature.title}
            >
      ...
      <PublicSection className={publicPageSection}>
        ...
        <div className="hide-scrollbar flex max-w-full snap-x snap-mandatory gap-5 overflow-x-auto pb-3 lg:gap-6" ref={templateRailRef}>
          {templateItems.map((template) => (
            <article
              className={`grid w-[18rem] flex-none snap-start overflow-hidden bg-white/90 sm:w-[19rem] lg:w-[20rem] ${publicCardShell}`}
              key={template.id}
            >
              <div className="bg-surface-container-low px-4 pb-4 pt-4 sm:px-5 sm:pt-5">
                <div className="overflow-hidden rounded-[1.25rem] bg-white p-3 shadow-ambient sm:p-4">
                  {template.previewImageUrl ? (
                    <img
                      className="h-[16rem] w-full rounded-[1rem] bg-white object-cover object-top sm:h-[17rem]"
              ...
              <div className="grid gap-4 p-5 text-center sm:p-6">
                ...
                <div className="pt-1">
                  <Button className="px-6" onClick={openHomeAuth} size="lg">
                    Use this template
                  </Button>
                </div>
              </div>
              ...
              {faqs.map((faq, index) => {
                ...
                return (
                  <article className={`overflow-hidden bg-white/90 ${publicCardShell}`} key={faq.question}>
                    ...
                    >
                      <span className="pr-4 font-headline text-xl font-extrabold leading-tight text-on-surface">
                        {faq.question}
                      </span>
                    ...
                    {open ? (
                      <div
                        className="border-t border-outline-variant/30 px-6 py-5 sm:px-7 sm:py-6 md:px-8"
