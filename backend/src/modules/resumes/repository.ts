...
export async function countResumes() {
  const result = await pool.query<{ count: string }>('SELECT COUNT(*)::text AS count FROM resumes');
  return Number(result.rows[0]?.count ?? 0);
}

export async function listResumes() {
  const result = await pool.query('SELECT * FROM resumes ORDER BY updated_at DESC, created_at DESC');
  return result.rows.map(mapRow);
}

export async function getResumeById(id: string) {
  const result = await pool.query('SELECT * FROM resumes WHERE id = $1 LIMIT 1', [id]);
...
export async function createResume(input: {
  ...
  const result = await pool.query(
    `INSERT INTO resumes (id, title, source, template_id, content_json, render_options_json, raw_text)
     VALUES ($1, $2, $3, $4, $5::jsonb, $6::jsonb, $7)
     RETURNING *`,
    [id, input.title, input.source, input.templateId, JSON.stringify(input.content), JSON.stringify(input.renderOptions), input.rawText ?? ''],
  );
...
export async function updateResume(id: string, input: Partial<{
  ...
  const current = await getResumeById(id);
